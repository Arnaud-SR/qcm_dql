...
class Questions {
    ...
    public function put(){
        ...
        $req = "INSERT INTO questions VALUES(DEFAULT, DEFAULT, {$ncx->esc($this->theme)},{$ncx->esc($this->title)},{$ncx->esc($this->$choices['A'])},{$ncx->esc($this->$choices['B'])},{$ncx->esc($this->$choices['C'])},{$ncx->esc($this->$choices['D'])},'{$right_answers}')";
        $cnx->xeq($req);
